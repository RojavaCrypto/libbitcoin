...
struct mlsag_signature
{
    ...
    bcs::ec_scalar initial_challenge;
    scalar_table salts;
};
...
void output_bytes(const auto& bytes)
{
    bool is_first = true;
    ...
    for (const auto& byte: bytes)
    {
        if (is_first)
            ...
            std::cout << ", ";
        std::cout << "0x" << std::hex << std::setw(2) << static_cast<int>(byte);
    }
}
    ...
    const point_ring& publics, const size_t index)
{
    ...
    for (size_t i = 0; i < rows_size; ++i)
        for (size_t j = 0; j < columns_size; ++j)
        {
            ...
            BITCOIN_ASSERT(hashed_publics[i][j].point()[0] != 0);
        }
    ...
    auto hashed_keys =
        digest(bcs::base16_literal("deadbeef"), publics);
    ...
    std::cout << "const ec_point G = ec_compressed{ ";
    output_bytes(G.point());
    std::cout << " };" << std::endl;
    std::cout << std::endl;

    std::cout << "using scalar_list = std::vector<bcs::ec_scalar>;"
        << std::endl;
    std::cout << "using scalar_table = std::vector<scalar_list>;"
        << std::endl;
    std::cout << "using apoint_list = std::vector<bcs::ec_point>;"
        << std::endl;
    std::cout << "using point_ring = std::vector<apoint_list>;"
        << std::endl;
    std::cout << std::endl;
    std::cout << "const scalar_table salts = {" << std::endl;
    bool is_first = true;
    std::ofstream outfile("data.bin");
    auto write_size = [&](auto value)
    {
        auto row_size_data =
            bcs::to_big_endian(static_cast<uint64_t>(value));
        outfile.write(reinterpret_cast<const char*>(row_size_data.data()),
            row_size_data.size());
    };
    ...
    write_size(rows_size);
    write_size(columns_size);
    for (const auto& scalar_column: signature.salts)
    {
        if (is_first)
            is_first = false;
        else
        ...
        for (const auto& salt: scalar_column)
        {
            if (is_first_1)
                is_first_1 = false;
            else
    ...
    std::cout << "const scalar_list challenges = {" << std::endl;
    for (auto it = challenges.begin(); it != challenges.end(); ++it)
    {
        const auto& challenge = *it;
        std::cout << "    ec_secret{ ";
        output_bytes(challenge.secret());
        write(challenge.secret());
        std::cout << " }";
        if (it != challenges.end() - 1)
            std::cout << ",";
        std::cout << std::endl;
    }
    std::cout << "};" << std::endl << std::endl;

    std::cout << "const point_ring publics = {" << std::endl;
    for (auto it = publics.begin(); it != publics.end(); ++it)
    {
        ...
        {
            std::cout << "        ec_compressed{ ";
            output_bytes(jt->point());
            write(jt->point());
            std::cout << " }";
            if (jt != points.end() - 1)
                std::cout << ",";
    ...
    std::cout << "const point_ring hashed_publics = {" << std::endl;
    for (auto it = hashed_publics.begin(); it != hashed_publics.end(); ++it)
    {
        const auto& points = *it;
        std::cout << "    {" << std::endl;
        for (auto jt = points.begin(); jt != points.end(); ++jt)
        ...
        std::cout << "    }";
        if (it != hashed_publics.end() - 1)
            std::cout << ",";
        std::cout << std::endl;
    }
    std::cout << "};" << std::endl << std::endl;

    std::cout << "const point_list key_images = {" << std::endl;
    for (auto it = signature.key_images.begin();
        it != signature.key_images.end(); ++it)
    {
        std::cout << "    ec_compressed{ ";
        output_bytes(it->point());
        write(it->point());
        std::cout << " }";
        if (it != signature.key_images.end() - 1)
            std::cout << ",";
        std::cout << std::endl;
    }
    std::cout << "};" << std::endl << std::endl;

    std::cout << "const point_ring left_points = {" << std::endl;
    for (auto it = left_points.begin(); it != left_points.end(); ++it)
    {
        ...
        std::cout << "    }";
        if (it != left_points.end() - 1)
            std::cout << ",";
        std::cout << std::endl;
    }
    std::cout << "};" << std::endl << std::endl;

    std::cout << "const point_ring right_points = {" << std::endl;
    ...
    std::cout << "salt[0][110] = "
        << convert_scalar(signature.salts[0][110]) << std::endl;
    ...
    std::cout << "key_images[0] = "
        << convert_point(signature.key_images[0]) << std::endl;
