    ...
    const point_ring& publics, const size_t index)
{
    ...
    auto compute_section = [&](size_t start_j, size_t end_j, auto skip_index)
    {
        for (size_t j = start_j; j < end_j; ++j)
        {
            ...
            for (size_t i = 0; i < rows_size; ++i)
                left_points[i][j] = signature.salts[i][j] * G;
        }
        for (size_t j = start_j; j < end_j; ++j)
        {
            if (j == skip_index)
        ...
        for (size_t j = start_j; j < end_j; ++j)
        {
            ...
            for (size_t i = 0; i < rows_size; ++i)
                right_points[i][j] += challenges[j] * signature.key_images[i];
        }
    };
    ...
    auto compute_section_old = [&](size_t start_j, size_t end_j, size_t skip_index)
    {
        {
            //std::lock_guard<std::mutex> lock(display_mutex);
            std::cout << "[" << start_j << ", " << end_j << "]" << std::endl;
        }
        for (size_t j = start_j; j < end_j; ++j)
        {
            if (j == skip_index)
    ...
    for (size_t thread_id = 0; thread_id < number_threads; ++thread_id)
    {
        ...
        if (thread_id == number_threads - 1)
            end_j = columns_size;
        threads.push_back(std::thread(compute_section_old, start_j, end_j, index));
    }
...
bool mlsag_verify(const point_ring& publics, const mlsag_signature& signature)
{
    ...
    auto compute_section_old = [&](size_t start_j, size_t end_j)
    {
        {
            //std::lock_guard<std::mutex> lock(display_mutex);
            std::cout << "[" << start_j << ", " << end_j << "]" << std::endl;
    ...
    for (size_t thread_id = 0; thread_id < number_threads; ++thread_id)
    {
        ...
        if (thread_id == number_threads - 1)
            end_j = columns_size;
        threads.push_back(std::thread(compute_section_old, start_j, end_j));
    }
